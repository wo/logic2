...
Let's define exactly what counts as a sentence in the standard language of modal
...
Consider `John must leave'. This would typically not be understood as a
...
Let's return to English expressions for necessity and possibility. These often
...
\begin{exercise}
  ...
  % Imagine you are given a ticket as a mark that you don't need to pay a fine.
  % But there is no rule that those without a ticket have to pay a fine. Then
  % [](~t -> f) is plausibly false, but ~t -> []f is true.
  % For the converse direction, normally we'd assume that it's a matter or law
  % that whoever doesn't have a ticket must pay a fine. So [](~t -> f) would
  % seem correct. In that case ~t -> []f often will also seem fine, because we
  % are restricting the domain of relevant worlds by the facts about whether you
...
If two operators stand in the relationship expressed by \pr{Dual1} and
\pr{Dual2}, they are called \textbf{duals} of each other. In modal logic,
...
$\Box \neg\neg A$. \pr{Dual1} then implies that for any $\L_{M}$-sentence $A$,
\[
   \Box A\text{ is equivalent to }\neg\Diamond\neg A.
\]
In the same manner, \pr{Dual2} implies that (for any $\L_{M}$-sentence $A$)
\[
...
The concept of duality can be extended from operators to sentences and sentence
...
To avoid confusion, we can put subscripts on the turnstile, so that
...
either an instance of \pr{T}, \pr{K}, \pr{4}, \pr{5}, or \pr{Dual}, or can be
generated from such instances by \pr{CPL} and \pr{Nec}. (In fact, \pr{4} is
redundant: any instance of \pr{4} can be derived from the remaining axioms an
rules.)

The system of modal logic defined by these schemas and two rules is perhaps the
...
Remember that a system of modal logic is just a set of $\L_{M}$-sentences that
implicitly defines a consequence relation. I have defined the system S5 in terms
of \pr{T}, \pr{K}, \pr{4}, \pr{5}, or \pr{Dual},
\pr{CPL} and \pr{Nec}, but the same system can be defined by many other
...
\pr{K} are in S5. In particular, we have
\[
