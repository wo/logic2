\chapter{Modal Predicate Logic}\label{ch:qml2}
...
This is where the concept of models comes in. A model contains just enough
information about a scenario and an interpretation to determine, for every
sentence and every world, whether the sentence is true at that world.

The non-logical vocabulary of $\L_{M\!P}$ consists of its names and predicates (with
...
being a sailor, the set of individuals to whom $F$ applies will differ from
...
other individuals besides Aristotle and Boethius. If yes, $\forall x Fx$ is
...
non-modal predicate logic. $\exists x \Diamond Fx$, for example, is true at a
...
\begin{proof}
  ...
  model $M$ relative to some assignment $g$, then $\Box \forall x A$ is also true
  at $w$ in $M$ relative to $g$. By clause (f) of definition
  \ref{def:constantdomainsemantics}, it follows that
  $\forall x \Box A \to\Box \forall x A$ is true at every world in every model
...
%
The resulting calculus is sound and complete for CK-validity.
...
the accessibility relation. The system \textbf{CT} is the set of
...
valid on all and only the frames whose accessibility relation has that
...
worlds, there are no humans, and no loaves of bread. So we should allow for
...
be false. But let's also assume that no individual in our world could have
been a unicorn. So $\forall x \Box \neg Ux$ is true. We now have a
...
A  downside of the possibilist interpretation is that it goes against the
...
Let's set aside these alternatives and see how our semantics could
...
\begin{definition}{}{variabledomainmodel}
  ...
  \item a non-empty set $W$ (the ``worlds''),
  \item a binary (``accessibility'') relation $R$ on $W$,
  \item a function $D$ mapping each world $w$ to a non-empty set $D_w$ (of ``individuals''), and
  \item an interpretation function $V$ that assigns
    \vspace{-1mm}
    \begin{itemize}
      \itemsep-1mm
...
$\Diamond \neg \exists x\, {x\!=\!b}$ is true at some world $w$. So
$\neg \exists x\, x\!=\!b$ should be true at some world $v$ accessible from $w$
-- a world $v$ where Loafy doesn't exist. The problem
is that we need to explain how a sentence that contains a name (here, $b$)
should be interpreted at a world (here, $v$) where the thing that's picked out
by the name doesn't exist.
...
predicate logic by dropping the assumption that every name has a referent. This
...
logic disagree. They hold that we can perfectly well reason with empty names. But we
need to answer the questions that I posed above: if $b$ is an empty
...
sentences with empty names are never true: if $b$ is empty, $Fb$ can't be
...
What shall we say about $\neg Fb$, if $b$ is an empty name? In some versions of
free logic, the standard semantic rules for complex sentences are applied: since
$Fb$ is not true, $\neg Fb$ is true, and so is $Fb \lor \neg Fb$. Other versions
of free logic assume that then neither $Fb$ nor $\neg Fb$ is true. Since a
sentence is called false iff its negation is true, this means that $Fb$ and
$\neg Fb$ are neither true nor false. We get a three-valued semantics. This can
be developed in different ways, with different verdicts on sentences like
$Fb \lor \neg Fb$.
...
approach, mainly because it is the simplest. That is, I will assume that at worlds where
...
that is valid in classical predicate logic is VK-valid. For example, both $b=b$
...
%
% We can't say "first" instead of "new" in the Existence rule because the first
% name might occur in a != a.
...
If so, we would have to conclude that Lois Lane has inconsistent beliefs,
...
\begin{solution}
  ...
  where $T$ translates `-- is a ticket' and $W$ translates `-- will win'.
...
\begin{solution}
  ...
  and $f(w)=g(w)$ then $f(v)=g(v)$. (We do not stipulate that if $wRv$ and
