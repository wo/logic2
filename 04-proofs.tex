...
all universal Kripke models. And so on.
...
``sequent calculi''. Logicians are liberal about what qualifies as a proof
...
\begin{definition}{}{correctstatement}
  Let $M = \t{W,R,V}$ be a Kripke model, and let $f$ be a function that maps world variables on a tree to members of $W$.
  A tree node of the form `$A \; (\omega)$' is a \textbf{correct statement about $M$ under $f$} iff $M, f(\omega) \models A$; a tree node of the form `$\omega R \upsilon$' is a correct statement about $M$ under $f$ iff the relation $R$ holds between $f(\omega)$ and $f(\upsilon)$.

  A tree branch \textbf{correctly describes} $M$ iff there is a function
...
\begin{proof}
  ...
  correctly describes $M$: all nodes on the branch are correct statements about
...
\begin{definition}{}{inducedmodel}
  ...
  \begin{itemize}[leftmargin=12mm]
    ...
    \item[(c)] for any sentence letter $\rho$, $V(\rho)$ is the set of world variables
          $\omega$ for which a node $\rho \; (\omega)$ occurs on the branch.
  \end{itemize}
\end{definition}
...
merely adding nodes that are already on the branch), then (ii)
...
\begin{solution}
  ...
  $\omega R \upsilon$. By clauses (b) and (h) of definition
...
\begin{proof}
  ...
  \itemsep0mm
    ...
    \item \pr{K} is the schema $\Box(A \to B) \to (\Box A \to \Box B)$. By
          ...
          $w$ while $\Box B$ is false. By clause (g) of definition
  ...
  \begin{enumerate}[leftmargin=9mm]
    ...
    \item By definition \ref{def:kripkesemantics}, the truth-functional
          ...
          sentences is also true at every world in every Kripke model.
...
\begin{definition}{}{cmk}
  ...
  \begin{itemize}[leftmargin=7mm]
    ...
    \item $wRv$ iff $v$ contains every sentence $A$ for which $w$ contains
          $\Box A$,
    \item for every sentence letter $\rho$, $V(\rho)$ is the set of all members of $W$
          that contain $\rho$.
  \end{itemize}
\end{definition}
...
\begin{proof}
  ...
  $A_0,A_1,\ldots$ be a list of all $\L_M$-sentences in some arbitrary
  order. For every number $i\geq 1$, define
  \[
