...
K-valid sentences can be derived from \pr{Dual} and \pr{K} by \pr{Nec} and
\pr{CPL}. The `all' part is completeness, the `only' part soundness. Having
...
which is either an instance of \pr{Dual} or \pr{K} or can be derived from
earlier sentences on the list by application of \pr{CPL} or \pr{Nec}. Expressed
as a construction rule, \pr{Nec} says that whenever a list contains a sentence
$A$ then one may append $\Box A$. \pr{CPL} says that one may append any
