...
If we read $r$ as `it is raining', we will use $\tF r$ to express that it will be
...
\begin{solution}
  ...
  \item $\neg \tF \tP p$ or, perhaps, $\tF\neg \tP p$\\
    $p$: There is a sea battle\\[-2mm]
  \item  $\tF(p \lor \tP p)$ or $\tF(\tF p \lor \tF\tP p)$\\
    $p$: It is warm\\[-2mm]
  \item $\neg \tP p \to \neg \tF q$ or $\tG(\neg\tP p \to \neg q)$\\
...
A complete scenario for temporal logic needs to tell us what is the case at each
time and which times come before which others. We represent such a scenario,
together with an interpretation of $\L_{t}$'s non-logical vocabulary, by a
structure that settles (a) what times there are, (b) how the times are ordered,
and (c) which sentence letters are true at which times. This is enough to
determine, for every $\L_{t}$-sentence and every time, whether the sentence is
true at that time.
...
We use `$M, t \models A$' as a shorthand notation to express that sentence $A$
...
defined in definition \ref{def:kripkemodel}. I have merely relabelled the set
`$W$' as `$T$', and the relation `$R$' as `$<$'. Definition
\ref{def:temporalsemantics} resembles definition \ref{def:kripkesemantics}
(``Kripke semantics''), except that we have two box-like operators $\tG$ and
$\tH$, and two diamond-like operators $\tF$ and $\tP$. The language of basic
temporal logic is bi-modal, with forward-looking operators ($\tF$ and $\tG$) and
backward-looking operators ($\tP$ and $\tH$). Unlike ordinary models for
multi-modal languages (definition \ref{def:multikripkemodel}), temporal models
have only a single accessibility relation. That's because the accessibility
relation for $\tP$ and $\tH$ is definable from the accessibility relation for
$\tF$ and $\tG$: a time $s$ is earlier than a time $t$ iff $t$ is later than
$s$.
...
earlier or later than another is relative to a spatial frame of reference. Models of relativistic time therefore often include a representation of
...
\begin{proof}
  ...
  $M$ in which the same sentences are true at $w$ as in $M$. Since $A$ is false
  at $w$ in $M$, it follows that $A$ is false at $w$ in $M^i$.
...
precedence relation does not branch in either direction. This is weaker than
connectedness, because it allows for frames with parallel time lines. There is
no schema that corresponds to connectedness.
...
\end{center}
\bigskip%
Note that these rules create three branches. The ``identity nodes''
...
while the past is ``settled''. Intuitively, it is open whether we will exercise
tomorrow, but not whether we have exercised yesterday. Some have argued that the
openness of the future calls for a non-linear model of time, with multiple
branches into the future. On one branch, we would exercise tomorrow, on another
we would not.

This line of thought, however, appears to conflate temporal and modal considerations.
...
follow from the Everett interpretation of quantum physics, according to which
chance events are really branching events in
...
If the laws are deterministic, the complete truth about the
...
% The models we are interested in are not forward-linear. I will, however, assume
...
maximal linearly ordered set of times later than $t$ will be called a
...
\end{exercise}
\begin{solution}
  Consider a model with three times ordered by $s<t$ and $s<r$. Assume $p$ is true at $t$ and not at $r$. Then $p \to \tH\tF p$ is false at $t$ on the Peircean interpretation.
\end{solution}

A different line is taken by (what Prior called) the \textbf{Ockhamist}
...
$p$ won't be the case. To talk about what will or won't be the case we must specify
which future we have in mind.

Formally, in Ockhamist semantics, the truth-value of every sentence is evaluated
at a pair consisting of a time and a history. Histories are linear by
...
says that every future contains a time at which $p$ is true; $\Diamond \tF p$, by
...
no sea battle. Let $p$ translate `there is a sea battle'. Is $\tF p$ true in
...
scenarios under all interpretations of non-logical vocabulary, we can't
...
conceivable scenario. In Ockhamist semantics, that parameter is the history $H$.
For a different application, consider vagueness. If $p$ translates `it is warm',
and the temperature is borderline warm, it is not clear what we should say about
the truth-value of $p$, and about various complex sentences containing $p$. One
...
extra parameter. Using the technique of supervaluation, we can define a
notion of truth without that parameter by saying that a sentence is true in a
scenario iff it is true in that scenario relative to every eligible sharpening.
...
\section{Extending the language}\label{sec:time2d}
...
A more powerful extension of $\L_T$ adds binary operators $\tS$ and $\tU$ for ``since'' and
...
Once we have $\tS$ and $\tU$, we don't need $\tF, \tG,\tP,\tH,\tX,$ and $\tY$ any more: they can all be defined in terms of $\tS$ and
...
The 2D semantics correctly predicts that $\tP\tN p$ entails $p$:
...
The presence of a ``Now'' operator has remarkable consequences. For example,
$\tN p \to p$ is valid, in the sense that it is true at all times in all models.
But $\tG(\tN p \to p)$ is invalid. If $p$ is true at $t$ and false at some time
after $t$, then $\tG(\tN p \to p)$ is false at $t$. So we must give up the
forward and backward Necessitation rules. The fact that something is logically
true does not entail that it is always going to be true!

We've met a version of all this before,
when we discussed the connection between apriority and metaphysical necessity in section \ref{sec:2d}.
There,
we saw that it might be useful to defined truth relative to pairs of worlds,
just as we've now defined truth relative to pairs of times.
The two applications of the 2D machinery are closely related.
In each case,
it reflects the fact that
when an operator shifts the point of evaluation
(to another time or world),
the evaluation of certain expressions that occur in the scope of that operator
(`now', `Aristotle')
can still depend on the point at which we started
(the present time, or the actual world).

\begin{exercise}
  `It could have been that everyone who is actually rich is poor.' This says
  ...
  you find a sentence $B$ in the resulting language that is logically true but not
