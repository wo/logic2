...
There are many kinds of norms: legal, moral, prudential, social, and so on. There may also be overarching norms that combine some
...
Now the subject (you) is mentioned twice in (2). A common assumption in deontic
...
system of norms, we can ask whether the history conforms to certain norms. Let's
...
everything that happens at an ideal world is plausibly permitted: it conforms to
the norms. The converse is plausible as well: whenever something is permitted
then it happens at some ideal world. For suppose an event doesn't happen at any
ideal world. Then the event entails the violation of some norm: it is
incompatible with the satisfaction of all norms. And then it can't be permitted.

All this suggests a simple possible-worlds analysis of permission:
\begin{quote}
...
holds between $w$ and $v$ does not depend on $w$. We have
\[
...
But the obligation can be overridden by exceptional circumstances or contrary
...
Seriality is weaker than reflexivity. Another weakening of reflexivity is
``shift-reflexivity''. $R$ is \emph{shift-reflexive} if $wRv$ implies $vRv$:
every world that can be seen can see itself. Shift-reflexivity corresponds to
the following schema \pr{U} (for ``utopia'')
%
...
world $w$ at which cyclists are required to wear top hats. Norman is cycling in
$w$, without a top hat. Is he violating the traffic laws? Well, he is violating
the laws of \emph{his} world, but not the laws of \emph{our} world. On an
absolutist approach, we interpret $\Ob$ and $\Pe$ as always referring to the
laws of our world (or some other fixed laws), no matter what world is under
consideration: $\Ob p$ is true at $w$ iff $p$ is required by the laws of our
world. On a relativist approach, we instead assume that $\Ob p$ is true at $w$
iff $p$ is required by the laws \emph{at $w$}.
...
deontic operator. Any sentence that does not contain an occurrence of $\Ob$ or $\Pe$ embedded under another occurrence of $\Ob$ or $\Pe$ is D-valid iff it is KD45-valid.
...
How could we account for this dependence on the circumstances? We can do so by
changing our interpretation of the accessibility relation.

Previously, we assumed that a world $v$ is accessible from $w$ iff all the norms
(or all the norms at $w$) are respected at $v$. Let's add another condition:
relevant circumstances at $w$ must also obtain at $v$. For example, if $w$ is a
world at which you come across a drowning baby then any accessible world must
also be a world at which you come across a drowning baby. In all ideal worlds
\emph{among these}, you rescue the baby.
...
worlds as accessible that differ from the actual world in matters that are
already settled. `Jones ought to be here', for example, can be true even if it's
settled that Jones is somewhere else. Perhaps the circumstantial accessibility
relation that figures in clause (a) varies with conversational context.
...
serious problem. It presupposes that the circumstantially accessible worlds
include ideal worlds at which no norms are violated. For suppose there are no
ideal worlds among the circumstantially accessible worlds. Then no world is
deontically accessible! We would have to conclude that everything is required
and nothing permitted. ($\Ob A$ is always true and $\Pe A$ false at worlds that
can't access anything.)

This is a serious problem because it is quite implausible that the
circumstantially accessible worlds always include ideal worlds. Suppose the
actual world is not ideal: it contains some norm violations. And suppose this is
settled: there's nothing anyone can do about it. Then we would expect that
all the circumstantially accessible worlds also contain some norm violations.
None of them is ideal.

% The problem is brought out by Arthur Prior's \emph{Samaritan Paradox}. Jones
% comes across a person who has been injured in a robbery. We want to say that
% Jones ought to help the injured victim: he does so in all deontically accessible
% worlds. But then the robbery must have taken place in all these worlds. (In a
% world without a robbery, there is no injured victim to help.)

% In Prior's example, it is settled that the robbery took place. All
% circumstantially accessible worlds contain the robbery. In a truly ideal world,
% nobody would have been robbed and nobody would be in need of help.

We need to revise our already-revised definition of deontic accessibility. How
could we do that?

Consider a concrete example, made famous by Arthur Prior. Jones comes across
Smith,q who has been injured in a robbery. We want to say that Jones ought to
help Smith: he does so in all deontically accessible worlds. But then the
robbery (or some analogous event) must have taken place in all these worlds.
Without the robbery, Smith wouldn't need help. When we say that Jones ought to
help Smith, we take the robbery as given; the robbery took place in all worlds
that are compatible with the ``relevant circumstances''. It follows that none of
these worlds are ideal. (In an ideal world, there are no robberies.) Crucially,
however, worlds where Jones doesn't help Smith are even \emph{worse}, in terms
of norm violations, than worlds where he helps. Both kinds of worlds are
non-ideal, because they both include the robbery. But our norms don't just
divide the possible worlds into ideal and non-ideal; they allow for finer
distinctions among non-ideal worlds. Jones ought to help the victim because
...
world-relative \emph{ordering} that tells us which worlds are better than others,
...
\begin{itemize}[leftmargin=10mm]
\itemsep-1mm
\item[(1)] If you smoke, you must smoke outside.
\item[(2)] If you miss the deadline for tax returns, you must pay a fine.
\item[(3)] If you have promised to call your parents, you must call them.
\end{itemize}
%
How could we translate these into our language $\L_M$? We seem to face a choice
between (W) and (N).
\begin{itemize}[leftmargin=10mm]
\itemsep-1mm
\item[(W)] $\Ob(p \to q)$
...
with the assumption that you are not required to smoke outside ($\neg \Ob q$), we surely
...
two argument places. Informally, $\Ob(B/A)$ means that $B$ ought to be the
...
\begin{exercise}\label{ex:chisholmsparadox}
  ...
  \end{exlist}
  \medskip\noindent%
  Intuitively, none of these four statements is entailed by the others,
  and they don't impose contradictory requirements on Jones. This shows
...
statements are restricted by circumstances. Return to Prior's example of Smith
and Jones. Smith is injured, Jones ought to tend to his injuries. It is
logically impossible to tend to injuries if there are no injuries. In all the
deontic logics we have so far considered, the claim that Jones ought to tend to
Smith's injuries therefore entails that Smith ought to be injured. But this
sounds wrong (as Prior pointed out).

Here, too, one might appeal to a pragmatic explanation. When we say that Jones
ought to tend to Smith's injuries, we take the injuries as fixed. We are
interested in what should be done \emph{given} the state in which Jones found
Smith. Worlds where Smith isn't injured are set aside; they are not
circumstantially accessible. But the criteria for circumstantial accessibility
can shift with conversational context. In particular, when we consider the claim
that Smith ought to be injured, we no longer hold fixed his injuries, as that
would make the claim pointless. We are no longer considering what should be done
given the state in which Jones found Smith, but whether that state itself should
have obtained. Worlds in which the state doesn't obtain become circumstantially
accessible.
...
quantification over possible worlds. By dropping this core tenet of Kripke
...
A popular alternative to Kripke semantics is \textbf{neighbourhood semantics},
...
of worlds in $N(w)$ is necessary at $w$.
...
which $A$ is true. This is our proxy for the proposition expressed by $A$. We then revise the clauses for the box and the diamond as follows.
%
%
\begin{definition}{Neighbourhood Semantics}{neighbourhoodsemantics}
    If $\Mfr = \t{W,N,V}$ is a neighbourhood model and $w$ is a member of $W$, then\\[1mm]
...
world without making $p\lor q$ necessary, even though $p$ entails $p \lor q$.
If we interpret $\Ob$ and $\Pe$ as the box and the diamond in neighbourhood
semantics, we can therefore easily say that Jones ought to tend to Smith's
injuries even though it is not the case that Smith ought to be injured.
...
Bare neighbourhood semantics determines a very weak logic called \textbf{E}.
This logic is axiomatized by \pr{Dual}, \pr{CPL}, and a rule (called ``RN'')
that allows inferring $\Box A \leftrightarrow \Box B$ from
$A \leftrightarrow B$. We can get stronger logics, with more validities, by
imposing conditions on the neighbourhood function $N$.
...
$\Ob A \to \Pe A$ requires that $N(w)$ doesn't contain contradictory
