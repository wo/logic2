...
if $w_{1}$ is a world at which it is currently raining. You don't know if light travels faster than sound in $w_{2}$. You also don't know what the sentence letters mean under
my interpretation. Does $p$ mean that it is raining? That light travels faster than sound? I haven't told you. Yet the sparse information I have given is
...
models in logic leave out important aspects of the scenarios and
interpretations they represent.

Given the simple possible-worlds analysis of the box and the diamond, we can
define a model for $\L_{M}$ as consisting of two parts. The first is a set of
things that we call ``worlds''. They don't need to be genuine worlds. They can
be arbitrary (usually not further specified) objects whose job is to represent
genuine worlds. The second part of a model is an ``interpretation function''
that tells us for each sentence letter at which of the worlds it is true.
...
I assume that you are familiar with elementary concepts of set theory. A \emph{set} is
...
that takes objects of a certain kind as input and returns objects of a possibly
different kind.
...
function $V$ tells us that $p$ is true at both worlds, $q$ is true at $w_{1}$,
...
\begin{enumerate*}
  ...
  \item $\Box\Diamond(\Box q \to \Diamond\Box p)$ is true at $w_{1}$ and $w_{2}$ (by 7
...
\begin{exercise}
  ...
  \emph{$\Gamma \models A$ iff there is no model throughout which all sentences in 
...
whenever a sentence $A$ is in S5, then so is $\Box A$. As we will show in chapter
\ref{ch:proofs}, these rules preserve validity (as defined in the previous
section). Indeed, we will show that the sentences that are
...
well as in section \ref{claim:replacement} in the previous chapter): that
...
\end{observation}
\begin{proof}
  \emph{Proof.} Remember that two sentences are logically equivalent iff each
  entails the other. So, by definition \ref{def:basicconsequence}: two sentences
  are equivalent iff they are true at the same worlds in every model.
  ...
  sentence. To show that $A$ and $A'$ are equivalent (our ``target hypothesis''), we first consider the case
  ...
  $B$). In the latter case, our assumption that the target hypothesis holds for
...
therefore start our construction by assuming that the \emph{negation} of
$\Diamond p \to \Box p$ is \emph{true} at some world $w$. We write this down as
follows.
...
which $p$ is true. Let's write that down.
...
and $u$. So $W = \{ w, v, u \}$. By line 4, $p$ is true at $v$. By line 5, $p$
...
Let's do another example. We'll check if $\Box(p\to q) \to (p \to \Box q)$.
That's another conditional, so we begin as before.
\begin{center}
  \tree[3]{%
...
One more example, before I state the general rules. This one is easy: we'll
check $\Box p \to \Diamond p$. You should be able to do the whole tree on your
own. Here it is.
%
\begin{center}
  \tree{%
    \nnodeticked{18}{1.}{$\neg(\Box p \to \Diamond p)$}{w}{(Ass.)}\\
    \nnode{18}{2.}{$\Box p$}{w}{(1)}\\
    \nnode{18}{3.}{$\neg\Diamond p$}{w}{(1)}\\
    \nnode{18}{4.}{$p$}{w}{(2)}\\
    \nnodeclosed{18}{5.}{$\neg p$}{w}{(3)}%
...
There is no open branch. What does
this tell us? It means that our search for a countermodel failed. We've unpacked
the assumption on node 1, that $\Box p \to \Diamond p$ is false at some world
$w$ in some model, and ran into a contradiction. So $\Box p \to \Diamond p$ is
true at all worlds in all models -- it is valid. In general, if all branches on
a tree close then the target sentence is valid.

Now for the general rules. To check whether a target sentence $A$ is valid, you
begin by assuming that the \emph{negation} of the target sentence is true at world $w$:
%
\begin{center}
  \tree{%
    \nnode{10}{1.}{$\neg A$}{w}{(Ass.)}
...
\end{center}
%
You then expand this node, and you continue expanding nodes that appear on
the tree, until no more nodes can be expanded.

To expand a node with a \emph{negated sentence} $\neg A$, you consider what the
falsity of the relevant sentence $A$ at the node's world implies for the
immediate parts of $A$. To expand a node with a \emph{non-negated sentence} $A$,
you consider what the truth of $A$ at the node's world implies for the
truth-value of the sentence's immediate parts. In either case, you append the
result to an open branch containing the expanded node.
...
The dotted line indicates the expansion step. The first rule, for example,
assumes that a node of the form $A \land B \;(\omega)$ occurs on a branch. This
prerequisite is written above the dotted line. If it is satisfied, you can
expand the node by adding what's below the dotted line to the branch containing
the expanded node: you add two nodes $A \;(\omega)$ and $B\;(\omega)$, one after
the other.

The rule for $\Box A$ says that nodes of the form $\Box A \;(\omega)$ are
expanded by adding $A \;(\nu)$, where $\nu$ is ``old''. By this I mean that
$\nu$ is any world variable that \emph{already occurs on the branch} to which
you add $A \;(\nu)$. You're not allowed to introduce a new world variable
(`$v$', `$u$', etc.) when expanding $\Box A$ nodes. The same is true for
...
introduce a new world variable: a variable that does not yet occur on the
branch.

If you have expanded a node that is not of type $\Box A$ or $\neg \Diamond A$,
and you have added the new nodes to every open branch containing the node, you
can tick off the node. You don't need to look at it again. This is obvious for
non-modal nodes. Expanding $A \land B \; (\omega)$ twice, for example, would
merely add another copy of $A\;(\omega)$ and $B\;(\omega)$ to the branch, and
repeated nodes (on the same branch) are always redundant. I have added a
checkmark next to the rules for $\Diamond A$ and $\neg \Box A$ as a reminder
that these nodes, too, can only be expanded once on each open branch, which is
less obvious: if you've already introduced a new world by expanding a
$\Diamond A$ or $\neg \Box A$ node, you're not allowed to introduce further new
worlds on the same branch by expanding the same node again.

$\Box A$ and $\neg \Diamond A$ are never ticked off. You can expand them
multiple times, for different ``old'' worlds. Even if you've expanded them for
every world currently on the branch, you leave them open, in case another world
variable appears in a later stage of the tree construction.

If, after expanding some node, a branch of a tree contains a sentence $A$ as
well as its negation $\neg A$, for the same world $\omega$, the branch must be
marked as \emph{closed} with an {\sffamily x} at the bottom. Closed
branches can not be extended any more.
...
If no more rules can be applied, the tree is complete. If all branches on the
completed tree are closed, the target sentence is valid. If some branch remains
open, the target sentence is invalid; any open branch then describes a
countermodel.
...
or $\neg(A \to B)$. Choose a node of another type only if none of these are
available. This heuristic often helps to keep trees small, but it is not part of
the official tree rules.

\begin{exercise}
  Use the tree method to check which of the following sentences are valid. If a
  sentence is invalid, spell out a countermodel, in addition to drawing the
  tree.
  \begin{exlist}
  \item $p\to q$  % a; invalid  
  \item $p \to \Box(p \lor q)$  % b; invalid
  \item $p \to \Box\Diamond p$  % c; valid
  \item $\Diamond \neg p \leftrightarrow \neg \Box p$  % d; valid 
  \item $\Diamond\Diamond p \to \Diamond p$  % e; valid
  \item $\Box p \lor \Box \neg p$  % f; invalid
  \item $\Diamond(p \to q) \to (\Diamond p \to \Diamond q)$  % g; invalid
  \item $\Diamond(p \land q) \to (\Diamond p \land \Diamond q)$  % h; valid
  \item $(\Diamond p \land \Diamond q) \to \Diamond(p \land q)$  % i; invalid 
  \item $\Diamond(p \lor q) \leftrightarrow (\Diamond p \lor \Diamond q)$  % j; valid
  \item $\Box \Diamond p \to p$  % k; invalid and infinite
  \end{exlist}
\end{exercise}
\begin{solution}
  (c), (d), (e), (h) and (j) are valid. You can find the trees at
  \href{https://www.umsu.de/trees/}{umsu.de/trees}. After entering a formula,
  tick the checkbox for `universal (S5)'.

  (a), (b), (f), (g), (i) and (k) are invalid. The trees and countermodels are as
  follows.
  ...
  \item[(g)] Target: $\Diamond(p \to q) \to (\Diamond p \to \Diamond q)$
  ...
  \item[(i)] $\neg ((\Diamond p \land \Diamond q) \to \Diamond(p \land q))$  % j
  ...
  \medskip\noindent
  We can choose either of the open branches to read off a countermodel. We get the same countermodel no matter which open branch we choose:
  $W = \{ w,v,u \}, V(p)=\{ v\}, V(q)=\{u\}$.
                
\medskip

    
  \item[(j)] $\Box \Diamond p \to p$
  ...
  \medskip\noindent The tree grows forever! The target sentence is invalid, but
...
To conclude, I want to mention two extensions of the tree method. The first
covers schemas. Have another look at the above tree for $\Box p \to \Diamond p$.
The tree closes. Now suppose we replace all occurrences of the sentence letter
$p$ throughout the entire tree with any other $\L_M$-sentence -- say,
$(p \lor \Diamond q)$. The resulting tree still conforms to our tree rules, and
it still closes. In effect, the tree therefore shows that all instances of the
schema $\Box A \to \Diamond A$ are valid. We could make this explicit by
directly using a schematic variable `$A$' instead of the sentence letter `$p$'.

In general, we can show that all instances of a schema are valid by drawing a
...
\begin{exercise}
  ...
  \par\nobreak % prevent excessive space.
  \begin{exlist}
...
\begin{solution}
  ...
  \href{https://www.umsu.de/trees/}{umsu.de/trees}. Remember to tick the checkbox for `universal (S5)'.
...
Finally, we can use the tree method to check whether some premises
$A_{1},\ldots,A_{n}$ entail some conclusion $B$. Remember from Observation
\ref{obs:semantic-deduction-theorem} that $A_{1},\ldots,A_{n}$ entail $B$ iff
$(A_1\land\ldots\land A_n) \to B$ is valid. To show that $A_{1},\ldots,A_{n}$
entail $B$ we could therefore draw a tree for $(A_1\land\ldots\land A_n) \to B$.
But we can save a few steps by starting the tree with multiple assumptions: one
for each of the premises $A_{1},\ldots, A_{n}$, and one for the negated
conclusion $\neg B$, all assumed to be true at the same world $w$. If the tree
closes, $A_1,\ldots,A_n$ entail $B$.

The following tree shows that $\Box p$ and $\Diamond(p \to q)$ together entail
$\Diamond q$.

\begin{center}
  \tree[3]{%
    & \nnode{16}{1.}{$\Box p$}{w}{(Ass.)} &&\\
    & \nnode{16}{2.}{$\Diamond (p \to q)$}{w}{(Ass.)} &&\\
    & \nnode{16}{3.}{$\neg \Diamond q$}{w}{(Ass.)} &&\\
    & \nnode{16}{4.}{$p\to q$}{v}{(2)} &&\\
    & \nnode{16}{5.}{$\neg q$}{v}{(3)} &&\\
    & \bnode{16}{6.}{$p$}{v}{(1)} &&\\
    &&& \\
    \nnodeclosed{8}{7.}{$\neg p$}{v}{(4)} && \nnodeclosed{8}{8.}{$q$}{v}{(4)} & 
  }%
\end{center}

\begin{exercise}
  Some have argued that the concept of God is incompatible with mere contingent
  existence: as a matter of necessity, \emph{if} God exists, then he exists
  necessarily. From the further premise that God \emph{could} exist, it is then
  inferred that he \emph{does} exist. Formalise this argument and use the tree
  method to check if it is valid.
\end{exercise}
\begin{solution}
  Let $p$ translate `God exists'. The premises are $\Box(p \to \Box p)$ and
  $\Diamond p$; the conclusion is $p$. The argument is valid, as \href{https://www.umsu.de/trees/#~8(p~5~8p),~9p|=p||universality}{this tree} shows.
\end{solution}
