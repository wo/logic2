...
In the next two chapters, we are going to add the resources of first-order
...
Atomic sentences of $\L_{P}$ are formed by combining a predicate with zero or
...
In English, a predicate is what you get when you remove all names from a
...
\begin{exercise}
  ...
  \item If a farmer owns a donkey, he beats it.
  \end{exlist}
\end{exercise}
\begin{solution}
  \begin{sollist}
  ...
  \item $\forall x \forall y((Fx \land Dy \land Oxy) \to Bxy)$; \qquad $F$: -- is a farmer, $D$: -- is a donkey, $O$: -- owns --, $B$: -- beats --
  \end{sollist}
\end{solution}
...
(d) by identifying the truth-value False with the empty set $\emptyset$ and the
truth-value True with the singleton set $\{ \t{} \}$ containing the empty tuple.
(Don't worry if you find this confusing or objectionable. We won't be using
zero-ary predicates.)
...
In this way, the truth-value of every closed atomic sentence is determined. For
...
first-order logic) iff it is true in all (classical, first-order) models.
Equivalently: An $\L_P$-sentence is valid iff it is true in all models relative
to all assignment functions. On this definition, $Fx \to Fx$ is valid, even
though it does not make a complete statement, due to the free variable $x$. Some
authors avoid this by restricting the concept of validity to closed sentences.
...
predicate a subset of $D$. Let's make this explicit:
...
(The two-dimensional semantics from section \ref{sec:2d} yields yet another
fragment of first-order logic, with two variables $x$ and $y$.)
...
\begin{solution}
  ...
  be a model with two worlds, each of which can see the other but not itself.
  Let all sentence letters be false at all worlds in both models. It follows
  that the very same $\L_M$-sentences are true at all worlds in these models (as
  a simple proof by induction shows). But the first model is reflexive and the
  second isn't. So there is no $\L_{M}$-question that is true at a world in a
  model iff the model's accessibility relation is reflexive.
\end{solution}
...
$a$ and $b$, we can add both $A[a/x]$ and $A[b/x]$. If there is no old name
...
-- so that, for example, $V(a) = a$ -- and that a predicate $P$ applies to a
...
\begin{principles}
  \pri{CPL}{\text{If }\Gamma \models_{0} A\text{ and all members of }\Gamma\text{ are on a proof, one may add $A$.}}\\
  \pri{Gen}{\text{If $A$ occurs on a proof, one may add $\forall \chi A[\chi/\eta]$.}}
\end{principles}
...
\begin{solution}
  ...
  \item %If anyone wants to leave early, they should do so quietly.
    ...
    All but the last of these are \emph{de re}. \\[-2mm]
...
is true iff the individual picked out by $a$ is the very same thing as the
...
individual, it never matters which of two names we use in a sentence as long as they pick out the same
...
occurs on a branch), we can always add a node $\eta=\eta$ to the branch.
Second, if an identity statement $\eta_1=\eta_2$ occurs on a branch, and some
sentence $A$ on the branch contains $\eta_1$, we may add a new node with
...
All this looks simple and unproblematic. But look what happens when we add boxes
and diamonds. Consider the following inference:
%
\begin{quote}
...
We will return to these puzzles in section \ref{sec:twi}. In the remainder of the
...
You have already encountered some uses of identity in earlier chapters. Suppose we want
...
\end{quote}
This has two readings. It can mean either that the actual Pope, Robert Francis Prevost, might have been Italian (\emph{de re}). Alternatively, it can mean
...
this one $F$ is also $G$. If we have an identity predicate, we can 
